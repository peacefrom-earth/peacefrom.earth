...
document.addEventListener('DOMContentLoaded', function() {
    ...
    drawWinnerBtn.addEventListener('click', async function() {
        try {
            ...
            if (result.winner) {
                winnerDisplay.textContent = `Winner: ${result.winner.email} - Wish: ${result.winner.wish}`; // Corrected syntax
            } else {
...
async function login(username, password) {
    console.log('Sending login request to:', `${SERVER_URL}/login`); // Corrected syntax
    const response = await fetch(`${SERVER_URL}/login`, { // Corrected syntax
        method: 'POST',
...
async function setup2FA() {
    console.log('Sending 2FA setup request to:', `${SERVER_URL}/setup-2fa`); // Corrected syntax
    const response = await fetch(`${SERVER_URL}/setup-2fa`, { // Corrected syntax
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include' // This is important for maintaining session cookies
    });
...
async function verify2FA(token) {
    console.log('Sending 2FA verification request to:', `${SERVER_URL}/verify-2fa`); // Corrected syntax
    const response = await fetch(`${SERVER_URL}/verify-2fa`, { // Corrected syntax
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
